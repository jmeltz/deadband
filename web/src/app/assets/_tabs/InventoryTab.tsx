...
import { useImportDBD } from "@/lib/hooks/useSites";
...
export function InventoryTab({ onDiscover }: { onDiscover?: () => void }) {
  // Filters
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="space-y-3">
        <div className="flex items-center gap-3">
          ...
          <FilterSelect
            ...
            placeholder="Status"
            options={["active", "retired", "quarantined"]}
          />
          <FilterSelect
            value={vulnFilter}
            onChange={setVulnFilter}
            placeholder="Vuln Status"
            options={["VULNERABLE", "POTENTIAL", "OK", "UNCHECKED"]}
          />
          {(vendorFilter ||
            siteFilter ||
            zoneFilter ||
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() => {
                ...
                checkAssets.mutate(ids ? { ids } : {});
              }}
              disabled={checkAssets.isPending || assets.length === 0}
            >
      ...
      {!isLoading && assets.length === 0 && (
        <EmptyState
          title="No assets"
          description="Discover devices on the network, then import them as assets to manage here."
        >
          {onDiscover && (
            <button
              onClick={onDiscover}
              className="text-xs text-db-teal-light hover:text-db-text transition-colors"
            >
              Go to Discover
            </button>
          )}
    ...
    >
      ...
      <td className="px-3 py-2 font-mono text-xs">
        <Link
          ...
          className="text-db-teal-light hover:text-db-text transition-colors"
...
function VulnStatusCell({ asset }: { asset: Asset }) {
  if (!asset.vuln_state) {
    return <span className="text-[10px] text-db-muted/50 italic">unchecked</span>;
  }
  ...
  return (
    <div className="flex items-center gap-1.5">
      <span
        className={`px-1.5 py-0.5 text-[10px] rounded-sm border ${statusColors[vs.status] ?? "bg-db-surface text-db-muted border-db-border"}`}
      >
        {vs.status}
      </span>
      {vs.risk_score > 0 && (
        <span className="text-[10px] font-mono text-db-muted">
          {vs.risk_score.toFixed(0)}
        </span>
      )}
      {vs.kev_count > 0 && (
        <span className="text-[10px] text-status-critical font-bold">KEV</span>
      )}
    </div>
  );
}
...
function DBDImportButton() {
  const inputRef = useRef<HTMLInputElement>(null);
  const importDBD = useImportDBD();
  const [result, setResult] = useState<string | null>(null);

  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    importDBD.mutate(file, {
      onSuccess: (data) => {
        setResult(
          `Imported: ${data.sites_imported} sites, ${data.assets_added} new, ${data.assets_updated} updated`,
        );
